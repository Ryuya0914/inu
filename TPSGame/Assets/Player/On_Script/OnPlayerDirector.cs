...
public class OnPlayerDirector : StrixBehaviour
{
    ...
    [SerializeField] OnPlayerMove S_Pmove;
    [SerializeField] OnPlayerGun S_Pgun;
    [SerializeField] OnPlayerTransChange S_Ptranschange;
    [SerializeField] OnPlayerFlag S_Pflag;
    [SerializeField] OnPlayerLife S_Plife;
    [SerializeField] OnPlayerAudio S_Paudio;
    OnPlayerUI S_Pui;
    [SerializeField] OnEffectController S_effect;           // 死亡
    ...
    void Awake() {
        if(isLocal) {
            ...
            // UIを見つけてくる
            S_Pui = GameObject.Find("PlayerCanvas").GetComponent<OnPlayerUI>();
        }
    ...
    void InputTransChage() {
        if(Input.GetMouseButtonDown(transchangeB)) {  // 変身の入力があるかどうか
            ...
            if(_data)
                ObjectDataLoad(_data);                   // オブジェクトデータ更新
        }
    }
